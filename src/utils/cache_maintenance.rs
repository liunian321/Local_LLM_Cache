...
pub async fn print_cache_stats(pool: &SqlitePool) -> Result<(), sqlx::Error> {
    ...
    println!("总缓存大小: {} 字节 ({:.2} MB)", total_size, total_size as f64 / (1024.0 * 1024.0));
    
    if !top_hits.is_empty() {
        println!("命中率最高的答案:");
        for (key, hits, size) in top_hits {
            println!("  Key: {}... | 命中次数: {} | 大小: {} 字节", key.chars().take(8).collect::<String>(), hits, size);
        }
    }
...
pub async fn cleanup_backup_table(pool: &SqlitePool) -> Result<(), sqlx::Error> {
    ...
    if exists_backup.is_some() {
        ...
        sqlx::query("DROP TABLE cache_backup")
            .execute(pool)
            .await?;
...
pub async fn cleanup_old_entries(
    ...
    // 删除过期的问题（但保留引用的答案）
    let deleted_questions = sqlx::query(
    ...
    println!("已清理 {} 条过期问题记录", deleted_questions.rows_affected());
